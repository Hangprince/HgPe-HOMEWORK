...
这周主要查找整理了一些资料，阅读了一些论文（例如：《中国传统民族乐器与西洋乐器的异同》）尝试搜集数据，但是民乐类的数据真的很难找，感觉是个不小的坑，我本想在艺术水平考级网上看看能不能找到全国报名民族乐器和西洋乐器的有多少人，但是未果。之后我在翻看先关调查报告（例如：《关于社会音乐考级的调查报告》）的时候，找到了一些调查的数据。后来我与同学讨论了一下，感觉可以换一个思路，不一定整篇都要要硬凹一个数据，可以把同宗的乐器进行对比可视化，比如二胡和小提琴的手法，钢琴和扬琴的音域等，做一个这方面的可视化。所以这几天在搜集这方面的资料。
